...
class TestDependentSCLS < Test::Unit::TestCase
  ...
  def test_default_paths
    default_locations = { :system => "/opt/rh/ruby200/root/usr",
                          :local  => "/opt/rh/ruby200/root/usr/local" }
    assert_equal default_locations, Gem.default_locations

    default_dirs = { :system => { :bin_dir => "/opt/rh/ruby200/root/usr/#{@bin_dir}",
                                  :gem_dir => "/opt/rh/ruby200/root/usr/share/gems",
                                  :ext_dir => "/opt/rh/ruby200/root/usr/lib64/gems" },
                     :local  => { :bin_dir => "/opt/rh/ruby200/root/usr/local/#{@bin_dir}",
                                  :gem_dir => "/opt/rh/ruby200/root/usr/local/share/gems",
                                  :ext_dir => "/opt/rh/ruby200/root/usr/local/lib64/gems" } }
  ...
  def test_paths_with_dependent_scl
    ...
    ENV['X_SCLS'] = "ruby200 #{scl_name}" # enabled scls
