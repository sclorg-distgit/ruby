...
# Use release tag directly dropping the %%{release_string} entirely
# as %%{?!foo: %%define foo bar} does not work for rpm < 4.14 on RHEL 7.
Release: 144%{?dist}
...
Source15: test_dependent_scls.rb

# Call load builtin macro if it is defined (RPM 4.12+),
# or call own load macro to avoid the error by "rhpkg srpm" on RPM 4.15+.
...
# Skip some clocks that are not available on Brew build environment.
# https://projects.engineering.redhat.com/browse/BST-1017
Patch100: ruby-2.7.0-skip-some-clocks.patch
# Skip the ssl tests for offline on Brew build environment.
# https://github.com/ruby/ruby/pull/4112
Patch101: ruby-3.0.0-Skip-ssl-tests-for-offline.patch

Requires: %{?scl_prefix}%{pkg_name}-libs%{?_isa} = %{version}-%{release}
Requires: %{?scl_prefix}ruby(rubygems) >= %{rubygems_version}
# We keep some default installed gems that is sub pacakge as Requires,
# focusing customer's use cases on RHEL.
...
# We don't add Requires redhat-rpm-config unlike Fedora to fix rhbz#1905222.
# Because we have been fine not including the package on SCL.
...
# StdLib default gems.
Provides: bundled(%{?scl_prefix}rubygem-did_you_mean) = %{did_you_mean_version}
Provides: bundled(%{?scl_prefix}rubygem-racc) = %{racc_version}
...
Provides:   bundled(%{?scl_prefix}rubygem-molinillo) = %{rubygems_molinillo_version}
BuildArch:  noarch
...
Requires:   %{?scl_prefix}ruby-default-gems >= %{ruby_version}
...
Provides:   bundled(%{?scl_prefix}connection_pool) = %{bundler_connection_pool_version}
Provides:   bundled(%{?scl_prefix}rubygem-fileutils) = %{bundler_fileutils_version}
Provides:   bundled(%{?scl_prefix}rubygem-molinillo) = %{bundler_molinillo_version}
Provides:   bundled(%{?scl_prefix}rubygem-net-http-persisntent) = %{bundler_net_http_persistent_version}
Provides:   bundled(%{?scl_prefix}rubygem-thor) = %{bundler_thor_version}
Provides:   bundled(%{?scl_prefix}rubygem-tmpdir) = %{bundler_tmpdir_version}
Provides:   bundled(%{?scl_prefix}rubygem-uri) = %{bundler_uri_version}
BuildArch:  noarch
...
%package -n %{?scl_prefix}rubygem-rbs
...
Requires:   %{?scl_prefix}ruby(release)
Requires:   %{?scl_prefix}ruby(rubygems) >= %{rubygems_version}
Provides:   %{?scl_prefix}rubygem(rbs) = %{version}-%{release}
BuildArch:  noarch

%description -n %{?scl_prefix}rubygem-rbs
...
%package -n %{?scl_prefix}rubygem-rexml
...
Requires:   %{?scl_prefix}ruby(release)
Requires:   %{?scl_prefix}ruby(rubygems) >= %{rubygems_version}
Provides:   %{?scl_prefix}rubygem(rexml) = %{version}-%{release}
BuildArch:  noarch

%description -n %{?scl_prefix}rubygem-rexml
...
%package -n %{?scl_prefix}rubygem-rss
...
Requires:   %{?scl_prefix}ruby(release)
Requires:   %{?scl_prefix}ruby(rubygems) >= %{rubygems_version}
Provides:   %{?scl_prefix}rubygem(rss) = %{version}-%{release}
BuildArch:  noarch

%description -n %{?scl_prefix}rubygem-rss
...
%package -n %{?scl_prefix}rubygem-typeprof
...
Requires:   %{?scl_prefix}ruby(release)
Requires:   %{?scl_prefix}ruby(rubygems) >= %{rubygems_version}
Requires:   %{?scl_prefix}rubygem(rbs) >= %{rbs_version}
Provides:   %{?scl_prefix}rubygem(typeprof) = %{version}-%{release}
BuildArch:  noarch

%description -n %{?scl_prefix}rubygem-typeprof
...
touch %{buildroot}%{_libdir}/gems/%{pkg_name}/bigdecimal-%{bigdecimal_version}/gem.build_complete
...
touch %{buildroot}%{_libdir}/gems/%{pkg_name}/io-console-%{io_console_version}/gem.build_complete
...
touch %{buildroot}%{_libdir}/gems/%{pkg_name}/json-%{json_version}/gem.build_complete
...
touch %{buildroot}%{_libdir}/gems/%{pkg_name}/openssl-%{openssl_version}/gem.build_complete
...
touch %{buildroot}%{_libdir}/gems/%{pkg_name}/psych-%{psych_version}/gem.build_complete
...
sed 's/@SCL@/%{scl}/' %{SOURCE15} > ./%{basename:%{SOURCE15}}
make test-all TESTS="%{basename:%{SOURCE15}}"
rm -rf ./lib/rubygems/defaults ./%{basename:%{SOURCE15}}
EOF}
...
%files -n %{?scl_prefix}rubygem-rbs
%{_bindir}/rbs
...
%files -n %{?scl_prefix}rubygem-rexml
...
%files -n %{?scl_prefix}rubygem-rss
...
%files -n %{?scl_prefix}rubygem-typeprof
