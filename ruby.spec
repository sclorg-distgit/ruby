...
Source2: test_dependent_scls.rb
...
# CVE-2013-4287: Fix RubyGems version regexp
# https://github.com/rubygems/rubygems/issues/626
Patch16: rubygems-2.1.0-version-regular-expression-dos.patch
# CVE-2013-4363: incomplete CVE-2013-4287 fix
Patch17: rubygems-2.1.5-CVE-2013-4363.patch
# glibc in on unexpected location for SCL. Use ldd instead.
# http://bugs.ruby-lang.org/issues/7828
Patch19: ruby-2.1.0-Get-libcs-path-by-ldd.patch
...
# - p21 is upstream revision
# - p22 is my fix for the revision (otherwise tcl/tk won't build)
# https://bugs.ruby-lang.org/issues/9386
Patch21: ruby-trunk-tk-rpath-fix.patch
Patch22: ruby-trunk-tk-extconf-fix.patch
# CVE-2014-4975: Fix off-by-one stack-based buffer overflow in the encodes() function
# https://bugs.ruby-lang.org/issues/10019
Patch28: ruby-2.1.0-CVE-2014-4975-fix-buffer-overru-by-tail_lf.patch
...
Patch29: ruby-2.0.0-p594-CVE-2014-8080-REXML-billion-laughs-attack.patch
# CVE-2014-8090: REXML incomplete fix for CVE-2014-8080.
# https://bugzilla.redhat.com/show_bug.cgi?id=1159927
Patch30: ruby-2.0.0-p598-CVE-2014-8090-REXML-incomplete-fix-for-CVE-2014-8080.patch
...
#%%{?scl:%%filter_from_provides s|pkgconfig(|%%{expand: %%{?scl_prefix}}pkgconfig(|g}
#%%{?scl:%%filter_setup}

# TODO: The filters above fails with RPM 4.11 with "warning: Ignoring invalid
# regex ^%{_scl_prefix}/.*$" error due to rhbz#982212. As a workaroung, all
# pkgconfig provides are filtered out and manualy provided by -devel
# sub-package, waiting for proper solution (potentionally blocked by
# rhbz#1023625) provided by scl-utils upstream.
%{?scl:%global __requires_exclude ^pkgconfig(.+)\s*=\s*(\d+\.?)+$}
...
# TODO: Related to the failing filters above. Should be autogenerated.
%{?scl:Provides: %{?scl_prefix}pkgconfig(%{pkg_name}) = %{ruby_abi}}
%{?scl:Provides: %{?scl_prefix}pkgconfig(%{pkg_name}-%{major_minor_version}) = %{ruby_abi}}
...
%%gem_extdir_mri %%{_libdir}/gems/exts/%%{gem_name}-%%{version}
...
rm %{buildroot}%{rubygems_dir}/rubygems/ssl_certs/ca-bundle.pem
...
# Disable test failing with tzdata-2014g. The upstream patch is overly complex.
# https://bugs.ruby-lang.org/issues/10245
sed -i '/^Europe\/Lisbon/ d' test/ruby/test_time_tz.rb
...
%changelog
* Wed Aug 12 2015 Vít Ondruch <vondruch@redhat.com> - 1.9.3.484-51
- Rebuild to enable Elliptic Curve Cryptography in the openssl extension.
  Resolves: rhbz#1116120
...
* Tue Nov 12 2013 Vít Ondruch <vondruch@redhat.com> - 1.9.3.448-41
- Use system libyaml for el7.
- Fix build against OpenSSL with enabled ECC curves.
  * ruby-1.9.3-p482-Fix-build-against-OpenSSL-with-enabled-ECC-curves.patch
  - Resolves: rhbz#1026993
- Fix test suite issue when compiling against glibc >= 2.17.
- Workaround pkgconfig provides issue.

* Mon Sep 23 2013 Josef Stribny <jstribny@redhat.com> - 1.9.3.448-40
- Fix CVE-2013-4363 (an incomplete fix of CVE-2013-4287)
  - Resolves: rhbz#1006440

* Mon Jul 08 2013 Josef Stribny <jstribny@redhat.com> - 1.9.3.448-39
- Fix CVE-2013-4287
  - Resolves: rhbz#1005269
